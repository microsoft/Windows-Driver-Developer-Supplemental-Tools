...
class ExtendedDeprecatedCall extends Element {
  ...
  ExtendedDeprecatedCall() {
    ...
    (
      ...
      // Functions marked deprecated in C28750
      (
        name.matches("lstrlen") and
        replacement =
          "_tcslen if the data is trusted, or _tcsnlen or StringCchLength if the data is untrusted"
        or
        name.matches("lstrlenA") and
        replacement =
          "strlen if the data is trusted, or strnlen or StringCchLengthA if the data is untrusted"
        or
        name.matches("lstrlenW") and
        replacement =
          "wcslen if the data is trusted, or wcsnlen or StringCchLengthW if the data is untrusted"
      )
      or
      // Functions marked deprecated in C28727
      (
...
where
  not deprecatedCall.getLocation().getFile().toString().matches("%Windows Kits%include%.h") and
  not deprecatedCall.getLocation().getFile().toString().matches("%.tmh") // Exclude autogenerated WPP files
