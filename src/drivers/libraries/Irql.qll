...
class KeLowerIrqlCall extends FunctionCall {
  ...
  int getIrqlLevel() {
    result =
      any(getPotentialExitIrqlAtCfnInternal(this.getMostRecentRaiseInterprocedural()
                .getAPredecessor())
      )
  }
        ...
        then result = cfn.(IrqlRestoreCall).getIrqlLevelExplicit()
          ...
          then result = cfn.(FunctionCall).getTarget().(IrqlRaisesAnnotatedFunction).getIrqlLevel()
            ...
            then result = getExplicitExitIrqlAtCfn(cfn.getAPredecessor())
              ...
              if exists(ControlFlowNode cfn2 | cfn2 = cfn.getAPredecessor())
              then result = getExplicitExitIrqlAtCfn(cfn.getAPredecessor())
              else result = getExplicitExitIrqlAtCfn(callerPredecessor(cfn.getControlFlowScope()))
            )
}
  ...
  exists(IfStmt ifStmt |
    ...
    (
      ...
      exists(LocalVariable v |
        ...
        not exists(AddressOfExpr ao | ao.getOperand().(VariableAccess).getTarget() = v)
      )
    )
  )
}
