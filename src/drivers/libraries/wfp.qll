...
class WfpMacro extends Macro {
  WfpMacro() {
    this.getFile().getBaseName() = ["fwpl.h", "Wfp_Annotations.h"] and
...
class WfpStreamInspection extends WfpAnnotation {
  WfpStreamInspection() {
    this.getMacro().(WfpMacro).getName() =
      ["_Wfp_stream_inspection_classify_", "_Wfp_stream_inspection_notify_"]
  }
}
...
class WfpFlowInspection extends WfpAnnotation {
  WfpFlowInspection() {
    this.getMacro().(WfpMacro).getName() =
      ["_Wfp_flow_inspection_notify_", "_Wfp_flow_inspection_classify_"]
  }
}
...
class WfpFlowInjection extends WfpAnnotation {
  WfpFlowInjection() { this.getMacro().(WfpMacro).getName() = ["_Wfp_flow_injection_classify_"] }
}
...
class WfpTransportInjection extends WfpAnnotation {
  WfpTransportInjection() {
    this.getMacro().(WfpMacro).getName() =
      ["_Wfp_transport_injection_classify_", "_Wfp_transport_injection_classify_inline_"]
  }
}
...
class WfpConnectRedirect extends WfpAnnotation {
  WfpConnectRedirect() {
    this.getMacro().(WfpMacro).getName() =
      ["_Wfp_connect_redirect_classify_", "_Wfp_connect_redirect_inline_classify_"]
  }
}
