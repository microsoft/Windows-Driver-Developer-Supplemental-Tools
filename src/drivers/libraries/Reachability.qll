 ...
 *
 * ```
 * class MyAnalysisConfiguration extends AbortReachability {
 ...
 */
abstract class AbortReachability extends string {
  bindingset[this]
  AbortReachability() { this.length() >= 0 }
 ...
 * Unlike `AbortReachability`, this analysis takes variable
 * reassignments into account.
 *
 * This class is used like `AbortReachability`, except that
 ...
 * addition to `reaches`.
 */
abstract class AbortReachabilityWithReassignment extends AbortReachability {
  bindingset[this]
  AbortReachabilityWithReassignment() { this.length() >= 0 }
  ...
  predicate reachesTo(
    ...
    exists(ControlFlowNode def |
      ...
      AbortReachability.super.reaches(def, v0, sink) and
  ...
  private predicate reassignment(
    ...
    AbortReachability.super.reaches(source, v, def) and
...
/**
 * Same as `AbortReachability`, but `isBarrier` works on control-flow
 ...
 * `AbortReachability`.
 */
abstract class AbortReachabilityExt extends string {
  bindingset[this]
  AbortReachabilityExt() { this.length() >= 0 }
