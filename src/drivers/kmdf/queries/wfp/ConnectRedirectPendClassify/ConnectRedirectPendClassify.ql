...
class FwpsPendClassifyCall extends Element {
  ...
  FwpsPendClassifyCall() { name = this.(FwpsPendClassifyApiCall).getTarget().getName() }
}
...
select waf,
  "A connect redirect callout " + waf.getName() +
    "called FwpsPendClassify0 and does not FWP_ACTION BLOCK or  FWPS_RIGHT_ACTION_WRITE flag."
