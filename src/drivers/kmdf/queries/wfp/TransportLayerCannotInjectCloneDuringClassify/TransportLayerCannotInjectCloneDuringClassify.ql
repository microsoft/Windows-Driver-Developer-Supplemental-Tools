...
predicate matchesInjectApi(string input) {
  input =
    any([
          "FwpsInjectTransportSendAsync", "FwpsInjectTransportSendAsync0",
          "FwpsInjectTransportReceiveAsync0", "FwpsInjectTransportReceiveAsync",
...
class TransportInjectCall extends Element {
  ...
  TransportInjectCall() { name = this.(FwpsInjectTransportApiCall).getTarget().getName() }
}

predicate matchesCloneApi(string input) {
  input =
    any([
          "FwpsAllocateNetBufferAndNetBufferList0", "FwpsAllocateCloneNetBufferList0",
          "FwpsAllocateDeepCloneNetBufferList0", "FwpsAllocateNetBufferAndNetBufferList",
          "FwpsAllocateCloneNetBufferList", "FwpsAllocateDeepCloneNetBufferList"
        ]
    )
}
...
class TransportCloneCall extends Element {
  ...
  TransportCloneCall() { name = this.(FwpsCloneApiCall).getTarget().getName() }
}
...
select waf,
  "Transport Injection Classify Function: " + waf.getName() +
    " tries to inject a cloned TCP packet. This is contract violation."
