...
class RelevantMemoryOrigin extends Element {
  RelevantMemoryOrigin() { this.(MemoryOrigin).originCanWrite() }
   ...
   */
  LocalAccess getAnAccess() { memoryOriginDataFlow(this, result) }
}
 ...
 * overlapping memory.
 */
predicate flowsToDereferencingAccessPair(
  AccessCandidate access1, UserModeDereferencingOperation deref1, AccessCandidate access2,
...
pragma[noinline]
predicate flowsToDereferencingAccess(
  AccessCandidate access2, UserModeDereferencingOperation deref2,
...
predicate alertCandidate(
  LocalAccess access1, UserModeDereferencingOperation deref1, UserModeDereferencingOperation deref2
) {
  flowsToDereferencingAccessPair(access1, deref1, _, deref2)
}

// deref1 and deref2 read the same bits if in a bit read expr.
predicate sameBitsIfBitRead(
  UserModeDereferencingOperation deref1, UserModeDereferencingOperation deref2
) {
...
pragma[noopt]
predicate alert(
  RelevantMemoryOrigin origin, LocalAccess access1, UserModeDereferencingOperation deref1
) {
...
from RelevantMemoryOrigin origin, LocalAccess access1, UserModeDereferencingOperation deref1
where alert(origin, access1, deref1)
select origin,
  "Buffer from " + origin.(MemoryOrigin).getADirectBufferAccess().getEnclosingFunction() + " line " +
    origin.getLocation().getStartLine() +
    " is dereferenced in '$@', which may be one of a pair of double fetches", access1,
  access1.getEnclosingFunction().getName() + " line " + access1.(Expr).getLocation().getStartLine() +
    " (" + access1 + ")"
