...
from
  ...
  ConditionalInitializationCall call, Evidence e
where
  exists(v.getARiskyAccess(f, call, e)) and
  e = DefinitionInSnapshot()
select call,
  "$@: The status of this call to $@ is not checked, potentially leaving $@ uninitialized.",
  call.getEnclosingFunction(), call.getEnclosingFunction().toString(), f, f.getName(), v,
  v.getName()
