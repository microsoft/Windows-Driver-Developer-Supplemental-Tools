  ...
  EscapingAllocation() {
    ...
    (
      ...
      exists(VariableAccess va | va = this.(StackAllocation).getAllocationVariable().getAnAccess() |
        ...
        exists(FunctionCall fc | fc.getTarget().getName().matches("%Probe%Write%") |
          fc.getArgument(1) = va
        )
        or
        /*
        ...
        exists(FunctionCall fc | fc.getTarget().hasName("VmbChannelPacketComplete") |
...
where
  ...
  not v.getAnAccess() =
    any(Call c | c.getTarget().getName().regexpMatch("mem(set|cpy)")).getArgument(0) and
  // The variable is never freed
  not v.getAnAccess() = any(Call c | c.getTarget().getName().regexpMatch("ExFree.*")).getArgument(0) and
  // Ignore stack variables assigned aggregate literals which zero the allocated memory
  not exists(AggregateLiteral al | v.getAnAssignedValue() = al |
    // = {}, which zeroes all bytes. Identified by the fact the aggregate has no children
    not exists(al.getAChild())
    or
    /*
