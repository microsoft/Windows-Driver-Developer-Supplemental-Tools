...
class LiteralLocalVariable extends LocalVariable {
  LiteralLocalVariable() {
    ...
    forex(Expr assigned | assigned = this.getAnAssignedValue() | assigned.isConstant()) and
   ...
   */
  Expr getALiteralExpr() { result = getAnAssignedValue() }
}
...
class LiteralAccess extends VariableAccess {
  LiteralAccess() { getTarget() instanceof LiteralLocalVariable }
   ...
   */
  string getALiteralValue() { result = getALiteralExpr().getValue() }

  /**
   * Gets an int representing one of the possible literal values for this access.
   */
  int getALiteralInt() { result = getALiteralValue().toInt() }
}
...
class OffsetOf extends AddressOfExpr {
  OffsetOf() { getAnOperand().(FieldAccess).getQualifier().getValue() = "0" }
}
