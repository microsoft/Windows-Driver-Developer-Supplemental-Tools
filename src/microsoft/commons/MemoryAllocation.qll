...
class ExAllocatePoolAllocation extends HeapAllocation {
  ExAllocatePoolAllocation() { this.getTarget().getName().matches("ExAllocatePool%") }

  override Expr getAllocatedSize() { result = this.getArgument(1) }
}
...
class UserAllocPoolAllocation extends HeapAllocation {
  UserAllocPoolAllocation() { this.getTarget().getName().matches("UserAllocPool%") }
