...
class UMAAccess extends VariableAccess {
  UMAAccess() {
    this =
      any(MacroInvocation e | e.getMacro().getFile().getBaseName().matches("usermode_accessors.h"))
          .getAnExpandedElement() or
    this =
      any(Call c | c.getTarget().getFile().getBaseName().matches("usermode_accessors.h"))
          .getAnArgument()
          .getAChild*()
  }
}
...
predicate inKernelModeIfStmt(Expr e) {
  ...
  |
    if eq instanceof EQExpr
    then ifStmt.getElse() = e.getEnclosingStmt().getParent+()
    else ifStmt.getThen() = e.getEnclosingStmt().getParent+()
  )
  or
  exists(IfStmt ifStmt, EqualityOperation eq |
    eq = ifStmt.getCondition() and
  ...
  |
    if eq instanceof EQExpr
    then ifStmt.getThen() = e.getEnclosingStmt().getParent+()
    else ifStmt.getElse() = e.getEnclosingStmt().getParent+()
  )
}

/** Holds if `va` occurs within a Microsoft style try-except block. */
predicate isInMicrosoftTry(VariableAccess va) {
  exists(MicrosoftTryStmt ts | ts.getStmt().getAChild+() = va.getEnclosingStmt())
}
...
class Memset extends FunctionCall {
  ...
  Expr getWriteTargetExpr() { result = getArgument(0) }
}
...
class MemoryCopy extends FunctionCall {
  ...
  Expr getCopyTargetExpr() { result = getArgument(0) }

  /** Gets the expr describing the source of the copy. */
  Expr getCopySourceExpr() { result = getArgument(1) }
...
predicate isWriteExprTarget(Expr e) {
  ...
  e =
    any(FunctionCall fc | fc.getTarget().getName().matches("%Probe%Write%"))
        .getAnArgument()
        .getAChild*() or
...
predicate isSpuriousRead(VariableAccess e) {
  e =
    any(FunctionCall fc | fc.getTarget().getName().matches("%ProbeFor%"))
        .getAnArgument()
        .getAChild*() or
  e =
    any(MacroInvocation m | m.getMacroName().matches("%ProbeFor%"))
        .getAnExpandedElement()
        .(Expr)
        .getAChild*() or
  e =
    any(FunctionCall fc | fc.getTarget().getName().matches("Mm%ecureVirtualMemory"))
        .getAnArgument()
        .getAChild*() or
  e =
    any(FunctionCall fc | fc.getTarget().getName().matches("ZwUnmapViewOfSection"))
        .getAnArgument()
        .getAChild*()
}

predicate isReadExpr(Expr e) { not isWriteExprTarget(e) and not isSpuriousRead(e) }
