...
class PointerToUserModeSysCallParameter extends AbstractPointerToUserModeSysCallParameter {
  PointerToUserModeSysCallParameter() { not this.getType().getName().matches("H%") }
}
...
class PointerToUserModeHandleParameter extends AbstractPointerToUserModeSysCallParameter {
  PointerToUserModeHandleParameter() { this.getType().getName().matches("H%") }
}
...
library class MaybeUserMemory extends Expr {
  final predicate comesFrom(Parameter p) {
    ...
    (
      ...
      any(MaybeUserMemory m | m.comesFrom(p)) = this.(VariableAccess).getQualifier()
      or
      // A function that is passed potential memory has potential memory.
      exists(Function f, Parameter param, int i |
        parameterUsePair(param, this) and
      ...
      exists(MaybeUserMemory def | definitionUsePair(_, def, this) | def.comesFrom(p))
      or
      // An object constructed from a user-mode handle.
      this.(ConstructorCall).getAnArgument() =
        any(MaybeUserMemory m |
