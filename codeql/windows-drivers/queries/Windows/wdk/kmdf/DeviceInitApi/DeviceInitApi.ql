...
class WdfInitializationApi extends Function {
  WdfInitializationApi() {
    this.getName() =
      [
        "WdfDeviceInitSetPnpPowerEventCallbacks", "WdfDeviceInitSetFileObjectConfig",
        "WdfDeviceInitSetExclusive", "WdfDeviceInitSetPowerPolicyEventCallbacks",
        "WdfDeviceInitSetPowerPolicyOwnership", "WdfDeviceInitRegisterPnpStateChangeCallback",
        "WdfDeviceInitRegisterPowerStateChangeCallback",
        "WdfDeviceInitRegisterPowerPolicyStateChangeCallback", "WdfDeviceInitSetIoType",
        "WdfDeviceInitSetPowerNotPageable", "WdfDeviceInitSetPowerPageable",
        "WdfDeviceInitSetPowerInrush", "WdfDeviceInitSetDeviceType", "WdfDeviceInitAssignName",
        "WdfDeviceInitAssignSDDLString", "WdfDeviceInitSetDeviceClass",
        "WdfDeviceInitSetCharacteristics", "WdfDeviceInitSetRequestAttributes",
        "WdfDeviceInitAssignWdmIrpPreprocessCallback", "WdfDeviceInitSetIoInCallerContextCallback",
        "WdfFdoInitSetFilter", "WdfFdoInitWdmGetPhysicalDevice", "WdfFdoInitOpenRegistryKey",
        "WdfFdoInitQueryProperty", "WdfFdoInitAllocAndQueryProperty", "WdfFdoInitSetEventCallbacks",
...
class WdfInitiailzationApiCall extends FunctionCall {
  WdfInitiailzationApiCall() { isWdfInitializationApiCall_aux(this) }
}
...
predicate isWdfInitializationApiCall_aux(FunctionCall fc) {
  ...
  exists(FunctionCall fc2 |
    fc2.getControlFlowScope() = fc.getTarget() and
    isWdfInitializationApiCall_aux(fc2)
  )
}
...
predicate isChildExpr(Expr e, FunctionCall func) {
  ...
  or
  exists(FunctionCall fc2 |
    fc2.getControlFlowScope() = func.getTarget() and
    isChildExpr(e, fc2)
  )
}
